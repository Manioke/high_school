...
class ExamPaperRequirement(Document):
    ...
    def after_insert(self):
        """Send email to HOD when the document is first generated in 'Awaiting Assignment' status."""
        if self.hod_user and self.status == "Awaiting Assignment":
            self.send_hod_assignment_email()

    def on_update(self):
        """Send email and notification to Lead Teacher when assigned or when lead_teacher_user changes."""
        before = self.get_doc_before_save()
        if self.lead_teacher_user and (not before or before.lead_teacher_user != self.lead_teacher_user):
    ...
    def request_changes(self, notes):
        ...
        recipients = [self.lead_teacher_user] + [row.user for row in self.collaborators if row.user]
        self.notify_users(recipients, _("Exam paper changes required"))
        
        # Outgoing Email Notification
        self.send_changes_requested_email(notes)
        return self.status
    ...
    def _get_user_emails(self, users):
        """Fetches valid RFC 5321 email addresses for a given user or list of users."""
        if not users:
            return []
        if isinstance(users, str):
            users = [users]

        user_ids = list({u for u in users if u})
        if not user_ids:
            return []
    ...
    def send_hod_assignment_email(self):
        """Sends an outgoing email to the HOD notifying them to assign staff."""
        recipients = self._get_user_emails(self.hod_user)
        if not recipients:
            return

        doc_link = frappe.utils.get_url_to_form(self.doctype, self.name)
        subject = _("Action Required: Assign Lead Teacher for {0}").format(self.requirement_title or self.name)

        message = f"""
            <p>Dear HOD,</p>
            <p>A new Exam Paper Requirement has been generated and is currently <b>Awaiting Assignment</b>:</p>
            <ul>
                <li><b>Title:</b> {self.requirement_title or self.name}</li>
                <li><b>Cycle:</b> {self.examination_cycle or 'N/A'}</li>
                <li><b>Course:</b> {self.course or 'N/A'}</li>
            </ul>
            <p>Please log in and assign a <b>Lead Teacher</b> and any necessary <b>Collaborators</b>.</p>
            <p><a href="{doc_link}" style="padding: 8px 12px; background-color: #1b408e; color: #fff; text-decoration: none; border-radius: 4px;">View Document</a></p>
        """
    ...
    def send_lead_teacher_assignment_email(self):
        """Sends an outgoing email to the Lead Teacher with dynamic deliverable requirements."""
        recipients = self._get_user_emails(self.lead_teacher_user)
        if not recipients:
            return

        doc_link = frappe.utils.get_url_to_form(self.doctype, self.name)
        subject = _("Assignment Notice: {0}").format(self.requirement_title or self.name)
        ...
        if requirements:
            req_list_html = "".join([f"<li style='margin-bottom: 5px;'><b>{req}</b></li>" for req in requirements])
        else:
            req_list_html = "<li>No specific files required for submission.</li>"

        message = f"""
            <p>Dear Teacher,</p>
            <p>You have been assigned as the Lead Teacher for <b>{self.requirement_title or self.name}</b>.</p>

            <p><b>Required Deliverables for Submission:</b></p>
            <ul>
                {req_list_html}
            </ul>

            <p>Please review the deadline details in the system and prepare the required documents accordingly.</p>
            <p><a href="{doc_link}" style="padding: 8px 12px; background-color: #1b408e; color: #fff; text-decoration: none; border-radius: 4px;">Open Requirement Form</a></p>
        """
    ...
    def send_hod_submission_email(self):
        """Sends an outgoing email to the HOD when the teacher submits the paper for review."""
        recipients = self._get_user_emails(self.hod_user)
        if not recipients:
            return

        doc_link = frappe.utils.get_url_to_form(self.doctype, self.name)
        submitter_full_name = frappe.db.get_value("User", frappe.session.user, "full_name") or frappe.session.user
        subject = _("Exam Paper Submitted for Review: {0}").format(self.requirement_title or self.name)

        submitted_deliverables = []
        if self.blueprint_file:
            submitted_deliverables.append("Blueprint File")
        if self.question_paper_file:
            submitted_deliverables.append("Question Paper File")
        if self.marking_scheme_file:
            submitted_deliverables.append("Marking Scheme File")
        if self.assessment_criteria:
            submitted_deliverables.append("Structured Assessment Criteria")

        if submitted_deliverables:
            deliverables_html = "".join([f"<li style='margin-bottom: 5px;'><b>{item}</b></li>" for item in submitted_deliverables])
        else:
            deliverables_html = "<li>No specific files attached.</li>"

        message = f"""
            <p>Dear HOD,</p>
            <p>Teacher <b>{submitter_full_name}</b> has submitted the examination paper for <b>{self.requirement_title or self.name}</b> for your review.</p>
            ...
            <p>Please review the attached documents and either approve the submission or request changes.</p>
            <p><a href="{doc_link}" style="padding: 8px 12px; background-color: #1b408e; color: #fff; text-decoration: none; border-radius: 4px;">Review Submission</a></p>
        """
    ...
    def send_exam_admin_approval_email(self, managers):
        """Sends an outgoing email to Education Managers when HOD approves a paper."""
        recipients = self._get_user_emails(managers)
        if not recipients:
            return

        doc_link = frappe.utils.get_url_to_form(self.doctype, self.name)
        hod_full_name = frappe.db.get_value("User", frappe.session.user, "full_name") or frappe.session.user
        subject = _("Approval Required: {0} Approved by HOD").format(self.requirement_title or self.name)

        message = f"""
            <p>Dear Exam Administration,</p>
            <p>The examination paper for <b>{self.requirement_title or self.name}</b> has been approved by HOD <b>{hod_full_name}</b> and is ready for final administration approval.</p>

            <ul>
                <li><b>Cycle:</b> {self.examination_cycle or 'N/A'}</li>
                <li><b>Course:</b> {self.course or 'N/A'}</li>
                <li><b>Lead Teacher:</b> {self.lead_teacher_user or 'N/A'}</li>
                <li><b>Revision Number:</b> {self.revision_number or 1}</li>
            </ul>

            <p><a href="{doc_link}" style="padding: 8px 12px; background-color: #1b408e; color: #fff; text-decoration: none; border-radius: 4px;">Open for Final Approval</a></p>
        """
    ...
    def send_changes_requested_email(self, notes):
        """Sends an outgoing email to Lead Teacher & Collaborators detailing required changes."""
        targets = [self.lead_teacher_user] + [row.user for row in self.collaborators if row.user]
        recipients = self._get_user_emails(targets)
        if not recipients:
            return

        doc_link = frappe.utils.get_url_to_form(self.doctype, self.name)
        reviewer_full_name = frappe.db.get_value("User", frappe.session.user, "full_name") or frappe.session.user
        subject = _("Changes Requested: {0}").format(self.requirement_title or self.name)

        formatted_notes = (notes or self.latest_review_notes or _("No specific notes provided.")).replace("\n", "<br>")

        message = f"""
            <p>Dear Teacher,</p>
            <p>Changes have been requested for the examination paper <b>{self.requirement_title or self.name}</b> by <b>{reviewer_full_name}</b>.</p>

            <p><b>Requested Changes / Details:</b></p>
            <blockquote style="margin: 10px 0; padding: 10px 15px; border-left: 4px solid #e24c4c; background-color: #f9f9f9; color: #333;">
                {formatted_notes}
            </blockquote>

            <p>Please log in, make the requested modifications, and resubmit to the HOD when completed.</p>
            <p><a href="{doc_link}" style="padding: 8px 12px; background-color: #1b408e; color: #fff; text-decoration: none; border-radius: 4px;">Open Examination Requirement</a></p>
        """
